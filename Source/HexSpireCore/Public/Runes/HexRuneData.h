...
struct HEXSPIRECORE_API FHexRuneTrigger
{
	 ...
	 *
	 * 语义：时机命中（且通过过滤器与条件）时累加内部计数，
	 *       计数达到 N 时【产生效果并清零】。
	 *
	 * ⚠️ 计数在【通过过滤器与条件之后】才累加。
	 *    否则"每移动 3 格触发"会被无关的位移事件提前撑满，
	 *    玩家数着格子却触发不了 —— §6.3 的示例符文
	 *    「你每移动 3 格，下一次攻击附加追击」完全依赖这个语义。
	 *
	 * ⚠️ 计数与 MaxPerRound/MaxPerBattle 是两套独立机制：
	 *    这个决定"攒够没有"，那个是 R7 的防自激硬上限。
	 */
